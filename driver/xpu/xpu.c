...
static inline u32 XPU_REG_TSF_CYCTIME_VAL_LOW_read(void){
	return reg_read(XPU_REG_TSF_CYCTIME_VAL_LOW_ADDR);
}

static inline u32 XPU_REG_TSF_CYCTIME_VAL_HIGH_read(void){
	return reg_read(XPU_REG_TSF_CYCTIME_VAL_HIGH_ADDR);
}

static inline u32 XPU_REG_WRITE_BEACON_VAL_LOW_read(void){
	return reg_read(XPU_REG_WRITE_BEACON_VAL_LOW_ADDR);
}

static inline u32 XPU_REG_WRITE_BEACON_VAL_HIGH_read(void){
	return reg_read(XPU_REG_WRITE_BEACON_VAL_HIGH_ADDR);
}

static inline u32 XPU_REG_READ_BEACON_VAL_LOW_read(void){
	return reg_read(XPU_REG_READ_BEACON_VAL_LOW_ADDR);
}

static inline u32 XPU_REG_READ_BEACON_VAL_HIGH_read(void){
	return reg_read(XPU_REG_READ_BEACON_VAL_HIGH_ADDR);
}

static inline u32 XPU_REG_RECEIVE_BEACON_VAL_LOW_read(void){
	return reg_read(XPU_REG_RECEIVE_BEACON_VAL_LOW_ADDR);
}

static inline u32 XPU_REG_RECEIVE_BEACON_VAL_HIGH_read(void){
	return reg_read(XPU_REG_RECEIVE_BEACON_VAL_HIGH_ADDR);
}

static inline u32 XPU_LEGACY_SIG_STB_VAL_LOW_read(void){
	return reg_read(XPU_LEGACY_SIG_STB_VAL_LOW_ADDR);
}

static inline u32 XPU_LEGACY_SIG_STB_VAL_HIGH_read(void){
	return reg_read(XPU_LEGACY_SIG_STB_VAL_HIGH_ADDR);
}
...
static inline u32 XPU_REG_WRITE_RESPONSE_VAL_LOW_read(void){
	return reg_read(XPU_REG_WRITE_RESPONSE_VAL_LOW_ADDR);
}

static inline u32 XPU_REG_WRITE_RESPONSE_VAL_HIGH_read(void){
	return reg_read(XPU_REG_WRITE_RESPONSE_VAL_HIGH_ADDR);
}

static inline u32 XPU_REG_READ_RESPONSE_VAL_LOW_read(void){
	return reg_read(XPU_REG_READ_RESPONSE_VAL_LOW_ADDR);
}

static inline u32 XPU_REG_READ_RESPONSE_VAL_HIGH_read(void){
	return reg_read(XPU_REG_READ_RESPONSE_VAL_HIGH_ADDR);
}

static inline u32 XPU_REG_RECEIVE_RESPONSE_VAL_LOW_read(void){
	return reg_read(XPU_REG_RECEIVE_RESPONSE_VAL_LOW_ADDR);
}

static inline u32 XPU_REG_RECEIVE_RESPONSE_VAL_HIGH_read(void){
	return reg_read(XPU_REG_RECEIVE_RESPONSE_VAL_HIGH_ADDR);
}

static inline u32 XPU_REG_SHORT_PREAMBLE_DETECTED_VAL_LOW_read(void){
	return reg_read(XPU_REG_SHORT_PREAMBLE_DETECTED_VAL_LOW_ADDR);
}

static inline u32 XPU_REG_SHORT_PREAMBLE_DETECTED_VAL_HIGH_read(void){
	return reg_read(XPU_REG_SHORT_PREAMBLE_DETECTED_VAL_HIGH_ADDR);
}

static inline u32 XPU_REG_LONG_PREAMBLE_DETECTED_VAL_LOW_read(void){
	return reg_read(XPU_REG_LONG_PREAMBLE_DETECTED_VAL_LOW_ADDR);
}

static inline u32 XPU_REG_LONG_PREAMBLE_DETECTED_VAL_HIGH_read(void){
	return reg_read(XPU_REG_LONG_PREAMBLE_DETECTED_VAL_HIGH_ADDR);
}
...
static int dev_probe(struct platform_device *pdev)
{
	...
	xpu_api->XPU_REG_WRITE_BEACON_VAL_LOW_read=XPU_REG_WRITE_BEACON_VAL_LOW_read;
	xpu_api->XPU_REG_WRITE_BEACON_VAL_HIGH_read=XPU_REG_WRITE_BEACON_VAL_HIGH_read;
	xpu_api->XPU_REG_READ_BEACON_VAL_LOW_read=XPU_REG_READ_BEACON_VAL_LOW_read;
	xpu_api->XPU_REG_READ_BEACON_VAL_HIGH_read=XPU_REG_READ_BEACON_VAL_HIGH_read;
	xpu_api->XPU_REG_RECEIVE_BEACON_VAL_LOW_read=XPU_REG_RECEIVE_BEACON_VAL_LOW_read;
	xpu_api->XPU_REG_RECEIVE_BEACON_VAL_HIGH_read=XPU_REG_RECEIVE_BEACON_VAL_HIGH_read;

	xpu_api->XPU_LEGACY_SIG_STB_VAL_LOW_read=XPU_LEGACY_SIG_STB_VAL_LOW_read;
	xpu_api->XPU_LEGACY_SIG_STB_VAL_HIGH_read=XPU_LEGACY_SIG_STB_VAL_HIGH_read;

	xpu_api->XPU_REG_TRX_STATUS_read=XPU_REG_TRX_STATUS_read;
	xpu_api->XPU_REG_TX_RESULT_read=XPU_REG_TX_RESULT_read;

	xpu_api->XPU_REG_WRITE_RESPONSE_VAL_LOW_read=XPU_REG_WRITE_RESPONSE_VAL_LOW_read;
	xpu_api->XPU_REG_WRITE_RESPONSE_VAL_HIGH_read=XPU_REG_WRITE_RESPONSE_VAL_HIGH_read;
	xpu_api->XPU_REG_READ_RESPONSE_VAL_LOW_read=XPU_REG_READ_RESPONSE_VAL_LOW_read;
	xpu_api->XPU_REG_READ_RESPONSE_VAL_HIGH_read=XPU_REG_READ_RESPONSE_VAL_HIGH_read;
	xpu_api->XPU_REG_RECEIVE_RESPONSE_VAL_LOW_read=XPU_REG_RECEIVE_RESPONSE_VAL_LOW_read;
	xpu_api->XPU_REG_RECEIVE_RESPONSE_VAL_HIGH_read=XPU_REG_RECEIVE_RESPONSE_VAL_HIGH_read;
	...
	xpu_api->XPU_REG_TSF_CYCTIME_VAL_LOW_read=XPU_REG_TSF_CYCTIME_VAL_LOW_read;
	xpu_api->XPU_REG_TSF_CYCTIME_VAL_HIGH_read=XPU_REG_TSF_CYCTIME_VAL_HIGH_read;

	xpu_api->XPU_REG_SHORT_PREAMBLE_DETECTED_VAL_LOW_read=XPU_REG_SHORT_PREAMBLE_DETECTED_VAL_LOW_read;
	xpu_api->XPU_REG_SHORT_PREAMBLE_DETECTED_VAL_HIGH_read=XPU_REG_SHORT_PREAMBLE_DETECTED_VAL_HIGH_read;

	xpu_api->XPU_REG_LONG_PREAMBLE_DETECTED_VAL_LOW_read=XPU_REG_LONG_PREAMBLE_DETECTED_VAL_LOW_read;
	xpu_api->XPU_REG_LONG_PREAMBLE_DETECTED_VAL_HIGH_read=XPU_REG_LONG_PREAMBLE_DETECTED_VAL_HIGH_read;
