...
.SYN_START_FLAG=false,.SYN_FIN_FLAG=true, .ptp_offset = 0, .sync_frame_index=0,\
...
.STA_session1_Nslots=4, .STA_session1_index = 0, .STA_session1_last_tsf = 0,\
.STA_session2_slots[0] = 2, .STA_session2_slots[1] = 3, \
.STA_session2_Nslots=1, .STA_session2_index = 0, .STA_session2_last_tsf = 0,\
.AP_session1_slots[0] = 1, .AP_session1_slots[1] = 2, .AP_session1_slots[2] = 3, .AP_session1_slots[3] = 4,\
.AP_session1_Nslots=4, .AP_session1_index = 0, .AP_session1_last_tsf = 0,\
.AP_session2_slots[0]=0, .AP_session2_slots[1]=1,\
.AP_session2_Nslots=1,.AP_session2_index = 0, .AP_session2_last_tsf = 0};
		       ...
		       struct sk_buff *skb)
{
	...
	// TODO: For AP, the queue_idx is determined by calculate_tx_queue and destination mac address. 
    ...
    if (Tdma_Node.is_AP == true){  // This is a beacon that is transmistted periodically
		if (is_beacon == true){
			...
			openwifi_set_tx_tsf(( beacon_indicator << 63)|(tx_timestamp)); // 0 indicates beacon

			printk("openwifi_tx: (Beacon) will be transmitted on %lld-th time slot in the %lld -th superframe\n",\
			calculate_slot_index(tx_timestamp, Tdma_Node.frame_begining_time), Tdma_Node.frame_index);	
		}
		else
		{
			...
			openwifi_set_tx_tsf(( beacon_indicator << 63 )|(tx_timestamp)); // 1 indicates others

			printk("openwifi_tx: fc_type %d fc_subtype %d will be transmitted on %lld -th time slot in the %lld -th superframe\n",\
			fc_type, fc_subtype, calculate_slot_index(tx_timestamp, Tdma_Node.frame_begining_time), Tdma_Node.frame_index);	
		}
	}
	else
	{
		...
		if (is_PTP_frame == true) {
			openwifi_set_tx_tsf(( beacon_indicator << 63 )|(tx_timestamp + 20));	
			Tdma_Node.log_tx_sta_syn_tsf = tx_timestamp + 20;
		}
		else
			openwifi_set_tx_tsf(( beacon_indicator << 63 )|(tx_timestamp + 520 + 20));	
	}
...
static irqreturn_t openwifi_rx_interrupt(int irq, void *dev_id)
{
	...
	struct openwifi_vif *vif_priv;
	struct ieee80211_mgmt *mgmt;
	u64 rx_time, ptp_offset, calculated_slot_offset,calculated_slot_index;
	...
	while( target_buf_idx_old!=target_buf_idx ) { // loop all rx buffers that have new rx packets
		...
		if (content_ok) {
			...
			if (skb) {
				...
				if (Tdma_Node.is_AP == true)
				{
					calculated_slot_index = calculate_slot_index(rx_time,Tdma_Node.frame_begining_time);
					calculated_slot_offset = div64_u64(rx_time - (Tdma_Node.frame_begining_time + mul_u64_u32_div(Tdma_Node.slot_duration ,calculated_slot_index , 1)), 10);
					...
					if((fc_type == 2 && fc_subtype == 13) && (fcs_ok) && (reverse16(addr2_high16) == 0x6655))
					{
						...
						, calculated_slot_index, Tdma_Node.frame_index,calculated_slot_offset);
					} 
					else
						...
						,fc_type, fc_subtype ,calculated_slot_index, Tdma_Node.frame_index, calculated_slot_offset,fcs_ok);
				}
				else // STA Actions
				{
					if ((fc_type == 0 && fc_subtype == 8) && (fcs_ok) && (reverse16(addr2_high16) == 0x6655)) // check whether it is a Beacon
					{
						...
						if (Tdma_Node.received_beacon_packet >= 100){
							if ((Tdma_Node.frame_index % Tdma_Node.STA_index == 1) && Tdma_Node.SYN_START_FLAG == false && Tdma_Node.SYN_FIN_FLAG == true )
							{
							...
							else if ((Tdma_Node.frame_index == Tdma_Node.sync_frame_index + 1) && Tdma_Node.SYN_START_FLAG == true  && Tdma_Node.SYN_FIN_FLAG == false) // PTP synchronization
							{   	
						...
						,fc_type, fc_subtype, calculate_slot_index(rx_time,Tdma_Node.last_rx_beacon_tsf), Tdma_Node.frame_index, fcs_ok);
				}	
...
static int openwifi_start(struct ieee80211_hw *dev)
{
	...
	xpu_api->XPU_REG_SLICE_COUNT_TOTAL_write(Tdma_Node.HW_frame_duration); // The total duration of a super frame  
	
	xpu_api->XPU_REG_SLICE_COUNT_START_write(0); //start 
	xpu_api->XPU_REG_SLICE_COUNT_END_write(Tdma_Node.session_0_ratio); //end 
	...
	xpu_api->XPU_REG_SLICE_COUNT_END_write( (1<<25) | (Tdma_Node.session_0_ratio + Tdma_Node.session_1_ratio) ); //end 
	...
	xpu_api->XPU_REG_SLICE_COUNT_END_write( (2<<25) | (Tdma_Node.HW_frame_duration) ); //end 
...
static enum hrtimer_restart openwifi_beacon_work(struct hrtimer *timer)
{
	...
	if (Tdma_Node.frame_index == 0)
		Tdma_Node.last_tx_beacon_tsf = openwifi_get_tsf_simp()+10000;
